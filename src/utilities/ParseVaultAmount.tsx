...
interface VaultAmtProps {
  amount?: bigint;
  size?: number;
  rounded?: boolean;
}

const ParseVaultAmount: React.FC<VaultAmtProps> = ({
  amount = BigInt(0),
  size = 16,
  rounded = false,
}) => {
  ...
  useEffect(() => {
    const fetchTokenPrice = async () => {
      try {
        const price = await getTokenPrice(ADDRESS.VAULT.SYMBOL.toLowerCase());
        setTokenPrice(price);
      } catch (error) {
  ...
  const VaultDecimals = ADDRESS.VAULT.DECIMALS;

  const formatToken = (num: bigint) => {
    const tokenValue = Dec(num, VaultDecimals);
  ...
  return (
    <span style={{ display: "inline-block", fontSize: size.toString() + "px" }}>
      {formatToken(amount)} {ADDRESS.VAULT.SYMBOL} (${formatInUSD(amount)} USD)
    </span>
  );
};

export default ParseVaultAmount;